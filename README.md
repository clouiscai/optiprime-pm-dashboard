...
The Inventory tab is generated from physical invoice purchase lines, so quantities stay synchronized with Spending. Purchased items can be dragged between Unsorted, Equipment, Tools, Hardware, Consumables, and Utilities. Non-consumable entries can be marked out of service and given a short status note. The Dashboard summarizes invoice spending by team and by Items, Services, and Team Enablement; item spending is further grouped by its Inventory classification.
